...
class LoginForm extends TPage
{
    ...
    function __construct($param)
    {
        ...
        $row=$table->addRow();
        $row->addCell( new TLabel('Bem-Vindo!', 'green') )->colspan = 2;
    ...
    public function onLogin()
    {
        try
        {
            ...
            $this->form->validate();
            $user = Usuario::authenticate( $data->lgn_usuario, $data->lgn_senha );
            if ($user)
            {
                TSession::regenerate();
                ...
                TSession::setValue('login', $data->lgn_usuario);
                TSession::setValue('userid', $user->lgn_id);
                TSession::setValue('usergroupids', $user->getSystemUserGroupIds());
                TSession::setValue('username', $user->lgn_usr_nome);
                TSession::setValue('frontpage', '');
                TSession::setValue('programs',$programs);
                
                if (!empty($user->unit))
                {
                    TSession::setValue('userunitid',$user->unit->lgn_id);
                }
